...
package body ECS.Event_Manager is
   ...
   package Event_Vectors is new Ada.Containers.Vectors
      (Index_Type => Natural, Element_Type => ECS.Event.Event_T);
   Event_Queue : Event_Vectors.Vector;

   --  Procedure to emit an event
   overriding procedure Emit_Event (Handler : in out Platform_Event_Handler;
      Event : ECS.Event.Event_T) is
   begin
      Event_Queue.Append (Event);
   end Emit_Event;

   --  Procedure to process all queued events
   overriding procedure Process_Events (Handler :
         in out Platform_Event_Handler) is
   ...
   overriding function Get_Next_Event (Handler :
         in out Platform_Event_Handler) return Event_T is
   begin
      if not Event_Queue.Is_Empty then
         declare
            First_Event : constant Event_T := Event_Queue.First_Element;
         begin
            Event_Queue.Delete_First;
            return First_Event;
         end;
