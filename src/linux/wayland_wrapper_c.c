...
// Callback for key events: keycode, pressed (1) or released (0) 
typedef void (*key_callback_t)(uint32_t keycode, uint32_t state);

// Callback for mouse button events: button, x, y, pressed (1) or released (0) 
typedef void (*mouse_button_callback_t)(uint32_t button, int x, int y, uint32_t state);

// Callback for mouse motion: x, y
typedef void (*mouse_motion_callback_t)(int x, int y);
...
struct wayland_window_context {
    ...
    /* Input devices */
    struct wl_seat *seat;               // Input device seat
    struct wl_keyboard *keyboard;       // Keyboard input
    struct wl_pointer *pointer;         // Mouse/pointer input
    ...
    uint32_t mouse_buttons;             // Pressed mouse buttons bitmask
    
    /* Input callbacks */
    key_callback_t key_callback;
    mouse_button_callback_t mouse_button_callback;
    mouse_motion_callback_t mouse_motion_callback;
};
    ...
    uint32_t version)
{
    ...
    if (strcmp(interface, "wl_compositor") == 0) {
        ...
        /* Bind to seat - for keyboard and pointer input */
        ctx->seat = wl_registry_bind(
            registry, name, &wl_seat_interface, 1);
        wl_seat_add_listener(ctx->seat, &seat_listener, ctx);
        printf("Bound to wl_seat\n");
    }
}
...
/*
 * ============================================================
 * Input Event Handlers (Keyboard and Pointer)
 * ============================================================
 */
                           ...
                           uint32_t format, int fd, uint32_t size)
{
    ...
    close(fd);  /* We're not using keymaps in this simple implementation */
}
                        ...
                        uint32_t state)
{
    ...
    /* Key event: key is the Linux keycode, state is 0 (release) or 1 (press) */
    /* Add 8 to convert evdev keycode to X11 keycode which matches Windows VK codes better */
    if (ctx->key_callback) {
        ctx->key_callback(key + 8, state);
    }
}
                               ...
                               uint32_t group)
{
    ...
    /* Modifier keys changed (Shift, Ctrl, Alt, etc.) */
}
                          ...
                          uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
    ...
    if (ctx->mouse_motion_callback) {
        ctx->mouse_motion_callback(ctx->mouse_x, ctx->mouse_y);
    }
}
                          ...
                          uint32_t state)
{
    ...
    if (state) {
        ctx->mouse_buttons |= (1 << (button - 0x110));
    } else {
        ctx->mouse_buttons &= ~(1 << (button - 0x110));
    }
    
    if (ctx->mouse_button_callback) {
        ctx->mouse_button_callback(button, ctx->mouse_x, ctx->mouse_y, state);
    }
}
...
static void seat_capabilities(void *data, struct wl_seat *seat, uint32_t capabilities)
{
    ...
    /* Check if seat has keyboard capability */
    ...
    const char* title)
{
    ...
    printf("Connected to Wayland display\n");
     ...
     // Get registry and discover compositor capabilities
    ...
    // Verify we got all required interfaces
    ...
    //Register XDG WM Base listener for ping events
    ...
    printf("Surface committed, waiting for configure...\n");
    
    /* Wait for configure event with roundtrip */
    ctx->configured = 0;
    wl_display_roundtrip(ctx->display);
    
    /* If not configured after roundtrip, try dispatching a few times */
    int attempts = 0;
    while (!ctx->configured && attempts < 10) {
        printf("Waiting for configure event (attempt %d)...\n", attempts);
    ...
    if (!ctx->configured) {
        fprintf(stderr, "Warning: Window not configured after %d attempts\n", attempts);
        /* Continue anyway - some compositors might not need it */
        ctx->configured = 1;
    }
    ...
    /* Attach the initial buffer and commit to make window visible */
    wl_surface_attach(ctx->surface, ctx->buffer, 0, 0);
    wl_surface_damage(ctx->surface, 0, 0, ctx->width, ctx->height);
    wl_surface_commit(ctx->surface);
    wl_display_flush(ctx->display);
    printf("Initial buffer attached and committed\n");
 ...
 * Unlike wayland_dispatch_event which blocks, this returns immediately
 * if no events are available.
 * 
 * This function:
 * 1. Flushes outgoing requests to the compositor
 * 2. Reads any incoming events from the socket (non-blocking)
 * 3. Dispatches all pending events in the queue
 * 
 * Returns: Number of events dispatched, or -1 on error
 */
int wayland_dispatch_pending(wayland_window_context* ctx)
{
    if (!ctx || !ctx->display) {
        return -1;
    }
    ...
    int ret = wl_display_dispatch_pending(ctx->display);
    if (ret < 0) {
        return -1;
    }
    
    /* Try to read new events from the socket (non-blocking) */
    /* prepare_read returns 0 if we can read, -1 if events are already queued */
    while (wl_display_prepare_read(ctx->display) != 0) {
        /* Events were queued while we tried to prepare, dispatch them */
        ret = wl_display_dispatch_pending(ctx->display);
        if (ret < 0) {
            return -1;
        }
    }
    
    /* Read events from socket (non-blocking via poll) */
    struct pollfd fds;
    fds.fd = wl_display_get_fd(ctx->display);
    fds.events = POLLIN;
    fds.revents = 0;
    
    int poll_ret = poll(&fds, 1, 0);  /* 0 timeout = non-blocking */
    ...
    if (poll_ret > 0) {
        ...
        /* No data available, cancel the read */
        wl_display_cancel_read(ctx->display);
    }
 ...
 * =====================
 * Copies pixel data from source buffer to Wayland shared memory
 * and commits the surface to display the changes.
 * 
 * Parameters:
 *   ctx - Window context
 *   buffer - Source pixel data (ARGB32 format, width*height*4 bytes)
 * 
 ...
 */
void wayland_set_key_callback(wayland_window_context* ctx, key_callback_t callback)
{
    if (ctx) {
 ...
 */
void wayland_set_mouse_button_callback(wayland_window_context* ctx, 
                                       mouse_button_callback_t callback)
{
    if (ctx) {
        ctx->mouse_button_callback = callback;
    }
}
 ...
 */
void wayland_set_mouse_motion_callback(wayland_window_context* ctx,
                                       mouse_motion_callback_t callback)
{
    if (ctx) {
        ctx->mouse_motion_callback = callback;
    }
}
