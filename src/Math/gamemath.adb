...
package body GameMath is
   ...
   --  Divide two vectors and return a new vector
   function "/" (V : Vec2; Val : Float) return Vec2 is
   begin
      return (X => V.X / Val, Y => V.Y / Val);
   end "/";
   ...
   procedure Normalize (V : in out Vec2) is
      Magnitude : constant Float := Sqrt (V.X * V.X + V.Y * V.Y);
   begin
      V := V / Magnitude;
   end Normalize;
   ...
   procedure Rotate (V : in out Vec2; Deg : Float) is
      ...
      New_X   : constant Float := V.X * Cos_Val - V.Y * Sin_Val;
      New_Y   : constant Float := V.X * Sin_Val + V.Y * Cos_Val;
   begin
      V.X := New_X;
      V.Y := New_Y;
   end Rotate;
